...
class MainActivity : AppCompatActivity() {
    ...
    private var changeModeListener = object : AdapterView.OnItemSelectedListener {
        override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
            changeMode(position)
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cameraIndex = sharedPref.getInt("camera_index", 0)
        spnCamera.setSelection(cameraIndex)
        initSpinner()
...
//        Toast.makeText(applicationContext, string, Toast.LENGTH_LONG).show()
    ...
    private fun changeCamera(position: Int) {
        with(sharedPref.edit()) {
            putInt("camera_index", position)
            commit()
        }
